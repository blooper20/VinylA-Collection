...
| # | 기능 | 웹 | 모바일 |
|---|------|-----|--------|
| 1 | 스피닝 다이어리 (Listening Log) | ✅ 완료 | ✅ 완료 (2026-07-15, 작성+목록+소셜+수정/삭제+미디어 첨부) |
| 2 | 오늘의 LP 추천 (랜덤 픽) | ✅ 완료 | ✅ 완료 (2026-07-15) |
| 3 | 오늘의 바이닐 스토리 (매거진) | ✅ 완료 (2026-07-14) | ✅ 완료 (2026-07-15) |
| 4 | 실시간 피드 (Discovery Feed) | ✅ 완료 (2026-07-14) | ✅ 완료 (2026-07-15) |
| 5 | 취향 매칭 & 유저 팔로우 | ✅ 완료 (2026-07-14) | ✅ 완료 (2026-07-15, 비공개 프로필/요청/목록/알림함 포함) |
...
- [x] 스토리 화면 (3) — StoryScreen + 피드 상단 배너 진입 (2026-07-15)
- [x] 피드 탭/화면 (4) — SocialScreen 피드 탭, useFocusEffect로 포커스 동안만 Realtime 구독 (2026-07-15)
- [x] 추천 수집가 레일 + 팔로우 버튼 (5) — 피드 탭 상단 가로 레일 (2026-07-15)
- [x] 프로필 화면 팔로우 버튼 (5) — UserProfileScreen 3상태 버튼 + 비공개 잠금 + 컬렉션/다이어리 탭 (2026-07-15)
- [x] 하단 탭 재구성 — 컬렉션(보관함+위시 헤더 배지 전환), 소셜(피드/다이어리), 스캔, 검색, 마이 (2026-07-15)

### 모바일에 아직 없는 것 (웹만)
- 알림 항목별 딥링크 세분화 — 좋아요/댓글 알림은 소셜 탭으로만 이동(해당 기록의 소셜 모달을 바로 열지는 않음)
- 다이어리 영상 트리밍 — 웹은 MediaRecorder 재녹화 방식, 모바일은 15초 초과 영상을 안내 후 거절 (갤러리 15초 이내 영상·카메라 촬영은 그대로 첨부 가능)

### 2026-07-15 모바일 파리티 2차 (LP픽 + 다이어리 완성)
- [x] 오늘의 LP 랜덤 픽 (2) — 보관함 상단 "오늘 뭐 듣지? 🎲" 배너 → RandomPickModal(가중 랜덤+연출) → DetailModal 다이어리 작성으로 연결
- [x] 다이어리 수정/삭제 — 소셜 탭 다이어리 항목 ⋮ 메뉴 → 공용 에디터(SpinLogEditorModal)/삭제 확인. updateSpinLog/deleteSpinLog 재사용
- [x] 미디어(사진/영상) 첨부 — 작성·수정 모두 지원. 이미지는 JPEG 재인코딩(HEIC 415 방지) 후 uploadSpinLogMedia(웹 API 라우트 공용), 영상은 15초/25MB 클라이언트 검증
- [x] 취향 일치율 배지 — UserProfileScreen 헤더, 웹 PublicGrid와 동일 정의(겹침 ÷ min(내, 상대) × 100, 겹침 0이면 미표시)
