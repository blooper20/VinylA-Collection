...
interface VinylSocialModalProps {
  ...
  onSummaryChange?: (id: number, summary: VinylSocialSummary) => void;
}

// 스피닝 다이어리 기록 상세 — 좋아요/댓글(답글)/공유/저장/신고.
// 상호작용 권한(볼 수 있는 기록인지)은 RLS가 판정하므로 여기서는 UI만 담당.
export const VinylSocialModal: React.FC<VinylSocialModalProps> = ({ entry, ownerName, onClose, onSummaryChange }) => {
  const { t } = useLocale();
  ...
  const handleReportLog = () => {
    if (!user) return showNotice(t('spinSocial.loginRequired'));
    setReportTarget({ id: id, type: 'vinyl' });
  };

  const handleReportComment = (commentId: number) => {
    if (!user) return showNotice(t('spinSocial.loginRequired'));
    setReportTarget({ id: commentId, type: 'vinylComment' });
  };
  ...
  const handleDeleteComment = async (comment: VinylComment) => {
    const removed = 1 + (comment.REPLIES?.length || 0);
    try {
      await deleteVinylComment(comment.COMMENT_ID);
      await loadComments();
  ...
  const renderComment = (c: VinylComment, isReply = false) => (
    <div key={c.COMMENT_ID} style={{ marginLeft: isReply ? '34px' : 0, padding: '8px 0' }}>
      ...
      {c.REPLIES && c.REPLIES.map((r: VinylComment) => renderComment(r, true))}
    </div>
  );
      ...
      >
        ...
        <div style={{ padding: '0 18px 14px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>
            {new Date(entry.ADDED_AT).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}
          </div>
        </div>
