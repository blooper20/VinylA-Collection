...
export const SideNav: React.FC = () => {
  ...
  // 마이페이지·알림은 메인 목록과 별개로 항상 같은 자리(하단)에 한 행으로
  // 나란히 고정 — 언어·로그아웃은 마이페이지 화면 안의 설정으로 옮겨갔다.
  const myPageItem: NavItem = { name: t('nav.my'), path: '/my', icon: 'person' };
  const notificationsItem: NavItem = { name: t('nav.notifications'), path: '/notifications', icon: 'notifications', badge: unreadCount };

  const adminNavItem: NavItem = { name: t('nav.admin'), path: '/admin', icon: 'admin_panel_settings' };

  const activeNavItems = [...navItems, ...(user?.app_metadata?.role === 'admin' ? [adminNavItem] : [])];
      ...
      <div className={styles.bottom}>
        <div className={styles.bottomRow}>
          <Link
            ...
            onClick={() => setExpanded(false)}
            aria-label={notificationsItem.name}
            className={`${styles.navItem} ${styles.bottomItem} ${isItemActive(notificationsItem) ? styles.active : ''}`}
          >
